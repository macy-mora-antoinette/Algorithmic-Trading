...
class AlgorithmicTrading(object):  		  	   		   	 		  		  		    	 		 		   		 		  
    ...
    def add_evidence(  		  	   		   	 		  		  		    	 		 		   		 		  
        ...
        self.learner = bt.BaggingTrees(learner = rt.RandomTrees, kwargs = {"leaf_size":5}, bags = 10, boost = False)   		  	   		   	 		  		  		    	 		 		   		 		  
    ...
    def testPolicy(  		  	   		   	 		  		  		    	 		 		   		 		  
        ...
        #create indicators as features in the x dataframe   
        bb = Bollinger(prices, 15) #Bollinger Bands
        gold = GoldenX(prices, 7, 14) #Golden Crossing
        macd = MACD(prices,10, 20) #Moving Avg Convergence-Divergence
        xdata = pd.concat([bb, gold, macd], axis = 1)
        ...
        for i in range(trades.size-1): 
            ...
            if trades[i] == 1: 
                if stocks_owned == 0: #and if you don't own any shares
                    stocks_owned += 1000                  
                    cost == self.commission + (self.impact*prices.iloc[i][symbol]*1000)
                    price == prices.iloc[i][symbol]*1000
                    df.iloc[i][symbol] += 1000 #buy new shares
                    if (price-price_old)*2 >= (cost+cost_old):                       
                        price_old = price
                        cost_old = cost
                        continue
                    ...
                    stocks_owned += 2000
                    cost == self.commission + (self.impact*prices.iloc[i][symbol]*2000)
                    price == prices.iloc[i][symbol]*2000
...
if __name__ == "__main__":  		  	   		   	 		  		  		    	 		 		   		 		  
    print("passed trading with impact")  		  	   		   	 		  		  		    	 		 		   		 		  
